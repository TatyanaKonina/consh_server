...
char chatHistory[CHATDATA][CHATDATA] = { 0 };
...
void* do_chat(void* arg)
{
	...
	while (1) {
		if (cli_count != 1) {
			...
			if (strstr(mate_name, "chat") != NULL) {
				while (1) {
					...
					if (strstr(chatData, escape)) {//if user want to leave
						...
						printf("%s", s1);
						strcpy(chatHistory[lastChatMess++], s1);//save mess in servers history
			...
			else if (strstr(mate_name, escape) == NULL) {// if user do not want to leave
...
void* printChatHistory(SOCKET client)
{
	...
	char clear_history[] = "Sorry, no previous messages. Start chattering right now\n";
	if (lastChatMess == 0) {
		send(client, clear_history, sizeof(clear_history), 0);// no history print mess
	}
	else {
		for (int i = 0; i < lastChatMess; i++) {
			send(client, chatHistory[i], sizeof(chatHistory[i]), 0);
		}
	}
